...
export default function CBActivation() {
   ...
   const handleAcceptRequest = async (userId, isApprove) => {
      ...
      const response = await fetch(`http://103.187.147.82:1337/api/v1/user/activation?userId=${userId}&isApprove=${isApprove}`, {
         method: 'POST',
         headers: {
      ...
      console.info(result)
      if (result.code === 200) {
         window.alert('User Activation Success!')
      } else {
         window.alert(`User Activation Error ${result.errors.message}`)
      }
   ...
   const handleGetUserActivationRequestList = async (page = 1, perPage = 5) => {
      const token = localStorage.getItem('token')
      const response = await fetch(`http://103.187.147.82:1337/api/v1/user/activation?page=${page}&perPage=${perPage}`, {
         method: 'GET',
         headers: {
            'Content-Type': 'application/json',
            'X-Auth': `Bearer ${token}`
