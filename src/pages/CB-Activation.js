...
export default function CBActivation() {
   ...
   const handleAcceptRequest = async (userId, isApprove) => {
      ...
      const response = await fetch(`http://103.117.57.225:1337/api/v1/user/activation?userId=${userId}&isApprove=${isApprove}`, {
         method: 'POST',
         headers: {
   ...
   const handleGetUserActivationRequestList = async (page = 1, perPage = 5) => {
      const token = localStorage.getItem('token')
      const response = await fetch(`http://103.117.57.225:1337/api/v1/user/activation?page=${page}&perPage=${perPage}`, {
         method: 'GET',
         headers: {
            'Content-Type': 'application/json',
            'X-Auth': `Bearer ${token}`
