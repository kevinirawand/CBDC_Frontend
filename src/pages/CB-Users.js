...
export default function CBUserPage() {
   ...
   const handleGetUserList = async (page = 1, perPage = 5) => {
      ...
      const response = await fetch(`http://103.117.57.225:1337/api/v1/user?page=${page}&perPage=${perPage}`, {
         method: 'GET',
         headers: {
   ...
   async function createUser(credentials) {
      const response = await fetch('http://103.117.57.225:1337/api/v1/auth/register', {
         method: 'POST',
         headers: {
            'Content-Type': 'application/json'
         },
                              ...
                              <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                                 <TableCell component="th" scope="row" padding="none">
                                    <Stack direction="row" alignItems="center" spacing={2}>
                                       <Avatar alt={name} src={`http://103.117.57.225:1337/static/images/profile/${profilePicture}`} />
