...
export default function IMAccountPage() {
   ...
   const handleRedeem = async (amount) => {
      ...
      const response = await fetch(`http://103.117.57.225:1337/api/v1/transaction/redeem`, {
         method: 'POST',
         headers: {
            'Content-Type': 'application/json',
            'X-Auth': `Bearer ${token}`
   ...
   const handleExchange = async (amount) => {
      ...
      const response = await fetch(`http://103.117.57.225:1337/api/v1/transaction/exchange/`, {
         method: 'POST',
         headers: {
            'Content-Type': 'application/json',
            'X-Auth': `Bearer ${token}`
