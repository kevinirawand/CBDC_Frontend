...
export default function IMAccountPage() {
   ...
   const handleRedeem = async (amount) => {
      ...
      const response = await fetch(`http://157.10.252.215:1337/api/v1/transaction/redeem`, {
         method: 'POST',
         headers: {
            'Content-Type': 'application/json',
            'X-Auth': `Bearer ${token}`
   ...
   const handleExchange = async (amount) => {
      ...
      const response = await fetch(`http://157.10.252.215:1337/api/v1/transaction/exchange/`, {
         method: 'POST',
         headers: {
            'Content-Type': 'application/json',
            'X-Auth': `Bearer ${token}`
      ...
      console.info(exchangeJSON)

      if (exchangeJSON.code === 200) {
         window.alert('Request Exchange to CB Success!')
      } else {
