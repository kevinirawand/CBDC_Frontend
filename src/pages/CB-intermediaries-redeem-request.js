...
export default function CBIntermediariesRedeemRequest() {
   ...
   const handleAcceptRequest = async (transactionId, isApprove) => {
      ...
      const response = await fetch(`http://103.117.57.225:1337/api/v1/transaction/redeem/intermediaries-confirmation?transactionId=${transactionId}&isApprove=${isApprove}`, {
         method: 'POST',
         headers: {
   ...
   const handleGetRedeemRequestList = async (page = 1, perPage = 5) => {
      const token = localStorage.getItem('token')
      const response = await fetch(`http://103.117.57.225:1337/api/v1/transaction/redeem/intermediaries?page=${page}&perPage=${perPage}`, {
         method: 'GET',
         headers: {
            'Content-Type': 'application/json',
            'X-Auth': `Bearer ${token}`
