...
export default function CBUserPage() {
   ...
   const handleGetIntermediariesList = async (page = 1, perPage = 5) => {
      ...
      const response = await fetch(`http://103.13.206.208:1337/api/v1/user/?page=${page}&perPage=${perPage}&role=IntermediariesALL`, {
         method: 'GET',
         headers: {
   ...
   const handleGetValidatorList = async (page = 1, perPage = 5) => {
      const token = localStorage.getItem('token')
      const response = await fetch(`http://103.13.206.208:1337/api/v1/user/validators?page=${page}&perPage=${perPage}`, {
         method: 'GET',
         headers: {
            'Content-Type': 'application/json',
            'X-Auth': `Bearer ${token}`
   ...
   async function setValidator() {
      ...
      const response = await fetch(`http://103.13.206.208:1337/api/v1/user/validators/${intermediariesUser}`, {
         method: 'PUT',
         headers: {
            'Content-Type': 'application/json',
            'X-Auth': `Bearer ${token}`
