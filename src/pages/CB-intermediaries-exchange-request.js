...
export default function CBIntermediariesExchangeRequest() {
   ...
   const handleAcceptRequest = async (transactionId, isApprove) => {
      ...
      const response = await fetch(`http://157.10.252.215:1337/api/v1/transaction/exchange/confirmation?transactionId=${transactionId}&isApprove=${isApprove}`, {
         method: 'POST',
         headers: {
   ...
   const handleGetRedeemRequestList = async (page = 1, perPage = 5) => {
      const token = localStorage.getItem('token')
      const response = await fetch(`http://157.10.252.215:1337/api/v1/transaction/exchange?page=${page}&perPage=${perPage}`, {
         method: 'GET',
         headers: {
            'Content-Type': 'application/json',
            'X-Auth': `Bearer ${token}`
