...
export default function StableCoins({ title, subheader, chartLabels, chartData, cbdcSum, rtgsAmount, amount, plus, colors, ...other }) {
   const chartOptions = useChart({
      plotOptions: { bar: { columnWidth: '100%' } },
      fill: { type: chartData.map((i) => i.fill), colors: [`${colors}`] },
      labels: chartLabels,
      colors: [`${colors}`],
      xaxis: { type: 'datetime' },
      tooltip: {
